<template>
    <div>
        ...
        <task-modal-component :parent="parentId" :showModal="showModal" :id="id" :action='action' @click="hideModal"></task-modal-component>
    </div>
</template>
...
export default {
    ...
    props: ['id', 'name', 'description', 'started', 'status','parent'],
    data : function() {
      return {
          'showModal' : false,
          'action' : '',
    ...
    methods : {
        getParentId(){
            ...
            if(this.parent === null){
                axios.get('../user/task/get/parent/'+this.id).then((response) => {
                    this.parentId  = response.data.data;
                });
            } else {
